...
<nav class="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900">
  ...
  <a href="https://flowbite.com/" class="flex items-center">
      ...
      <span class="self-center text-xl font-semibold whitespace-nowrap dark:text-white">Fadetaig</span>
  </a>
  <div class="flex md:order-2">
      <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" @click="ambildata()"> Ambil Data </button>
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
  <table class="w-full text-sm text-left text-blue-100 dark:text-blue-100 text-xs text-white uppercase bg-blue-600 dark:text-white">
    <tr>
      <td scope="col" class="px-6 py-3">Waktu</td>
      <td scope="col" class="px-6 py-3">Suhu</td>
    </tr>
    <tr v-for="item in listsuhu">
      <td scope="col" class="px-6 py-3">{{ item.waktu }}</td>
      <td scope="col" class="px-6 py-3">{{ item.suhu }}</td>
    </tr>
  </table>
</div>
</template>
